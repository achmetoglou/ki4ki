...
Archiv-PDF — und zuletzt die Datei im Lösch-Ordner. Jeder Schritt steht in
`dokumente/<bereich>/loeschen.log`.

**Nur dieser Ordner löst ein Löschen aus.** Eine aus dem Archiv verschwundene
PDF löscht nie etwas. Schlägt die Schnittstelle fehl, bleibt die PDF im
Lösch-Ordner liegen und es wird eine Minute später erneut versucht.
Schalter: `KI4KI_LOESCHEN` (Standard an).
