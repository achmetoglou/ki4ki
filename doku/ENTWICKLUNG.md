...
Kopf; die **Reihenangabe** (`band_aus_text`) ebenfalls ohne Modell aus dem Vorspann,
im Wortlaut: `Band 400` (Impressum `Band:` oder Deckblatt `BAND`), `Reihe 20, Nr. 456`
(VDI-Fortschrittberichte), `Heft 23`, `Volume 12`. Drei Stufen, absichtlich eng: mit
Trennzeichen oder in Großschreibung immer; ohne Trennzeichen (`Heft 23`) und bloße
Nummern (`Nr. 456`) nur, wenn eine Reihe ausdrücklich genannt ist (`_REIHE_NAH`).
Gemessen: Ein weiter gefasstes Muster fing „noch" (19×), „vollständig" (9×),
„November" und „Teilchen"; `Teil` bleibt deshalb draußen — `DVS 2213-1 Teil 2` ist ein
Normteil, kein Band. Bestehende Einträge rüstet `_band_nachruesten` nach; `BAND_FASSUNG`
sorgt dafür, dass ein erweitertes Muster sie erneut ansieht. Titel, Verfasser und Jahr
liest das kleine Modell aus dem Inhalt (nicht aus dem
...
|---|---|---|
| `python3 pruef-proxy/dialogtest.py` | 35 Szenarien, 396 Prüfungen ohne Modell: Router, Faden, Prüfungskatalog, Rolle, Kategorien, Rechte je Ausgabeweg, Aufnahme-Übersicht | vor jedem Push |
